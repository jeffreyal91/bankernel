...
import com.bankernel.service.ContaBancariaQueryService;
...
/**
 * REST controller for managing {@link com.bankernel.domain.ContaBancaria}.
 */
@RestController
@RequestMapping("/api/conta-bancarias")
public class ContaBancariaResource {

    private static final Logger LOG = LoggerFactory.getLogger(ContaBancariaResource.class);
    ...
    public ResponseEntity<ContaBancariaDTO> createContaBancaria(@Valid @RequestBody ContaBancariaDTO contaBancariaDTO)
        ...
        contaBancariaDTO = contaBancariaService.save(contaBancariaDTO);
        return ResponseEntity.created(new URI("/api/conta-bancarias/" + contaBancariaDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, contaBancariaDTO.getId().toString()))
    ...
    public ResponseEntity<ContaBancariaDTO> updateContaBancaria(
        ...
        contaBancariaDTO = contaBancariaService.update(contaBancariaDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, contaBancariaDTO.getId().toString()))
            .body(contaBancariaDTO);
    }
    ...
    public ResponseEntity<ContaBancariaDTO> partialUpdateContaBancaria(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, contaBancariaDTO.getId().toString())
        );
    }
